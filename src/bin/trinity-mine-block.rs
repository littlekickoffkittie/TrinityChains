...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Print mining header
    println!("\n╔═══════════════════════════════════════════════════════════════╗");
    println!("║              ⛏️  MINING BLOCK {}                          ║", new_height);
    println!("╚═══════════════════════════════════════════════════════════════╝\n");
    ...
    let reward_triangle = chain.state.utxo_set.values()
    ...
    println!("│ Timestamp:           {:<66} │", new_block.header.timestamp);
    println!("│ Difficulty:          {:<66} │", new_block.header.difficulty);
    println!("│ Nonce:               {:<66} │", new_block.header.nonce);
    println!("│ Mining Time:         {:.3} seconds{:<57} │", elapsed.as_secs_f64(), "");
    println!("└────────────────────────────────────────────────────────────────────────────────────────┘\n");

    println!("┌──────────────────────────────── 🔺 REWARD TRIANGLE ───────────────────────────────────┐");
    println!("│ Reward Type:         Coinbase Transaction                                             │");
    println!("│ Amount:              1000.000000 TRC (geometric area units)                          │");
    ...
    if let Some(triangle) = reward_triangle {
        ...
        println!("│ Triangle Area:       {:.6}                                          │", area);
        println!("│ Vertices:            A({}, {}), B({}, {}), C({}, {})  │", 
            triangle.a.x.to_num::<f64>(), triangle.a.y.to_num::<f64>(),
            triangle.b.x.to_num::<f64>(), triangle.b.y.to_num::<f64>(),
            triangle.c.x.to_num::<f64>(), triangle.c.y.to_num::<f64>());
    } else {
        println!("│ Triangle Hash:       Queued for next confirmation                                   │");
        println!("│ Status:              NEW - Created this block                                     │");
    }
    println!("└────────────────────────────────────────────────────────────────────────────────────────┘\n");

    println!("┌────────────────────────────────── 📊 NETWORK STATE ─────────────────────────────────────┐");
    println!("│ Total Blocks:        {:<65} │", chain.blocks.len());
    println!("│ Total UTXOs:         {:<65} │", chain.state.utxo_set.len());
    println!("│ Transactions:        {:<65} │", new_block.transactions.len());
    println!("│ Chain Difficulty:    {:<65} │", chain.difficulty);
    println!("│ Your Balance:        {} TRC (from {} triangles)                    │", 
        chain.state.get_balance(&address).to_num::<f64>(),
        chain.state.utxo_set.values().filter(|t| t.owner == address).count());
    println!("└────────────────────────────────────────────────────────────────────────────────────────┘\n");
